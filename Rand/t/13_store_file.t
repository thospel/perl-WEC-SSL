...
# `make test'. After `make install' it should work as `perl -T 13_store_file.t'
#########################
...
my @methods = qw(store_file RAND_DATA);
...
umask 0322;
$written = WEC::SSL::Rand::store_file($out_file);
...
eval { WEC::SSL::Rand::store_file($out_file . $taint) };
...
$written = eval { WEC::SSL::Rand::store_file("$out_dir") };
like($@, qr/^Path '\Q$out_dir\E' exists but is not a regular file at /si);
rmdir($out_dir) || $! == ENOENT || die "Could not rmdir($out_dir): $!";
