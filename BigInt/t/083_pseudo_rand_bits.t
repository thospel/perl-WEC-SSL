...
# `make test'. After `make install' it should work as `perl -T 083_pseudo_rand_bits.t'
#########################
our $VERSION = "1.000";
...
my @methods = qw(pseudo_rand_bits);
...
$result = eval { Big->pseudo_rand_bits(bits => -1) };
like($@, qr/^Negative number of bits at /i);

$result = eval { Big->pseudo_rand_bits(floepr => 28) };
like($@, qr/^Unknown option 'floepr' at /i);

$result = eval { Big->pseudo_rand_bits("floepr") };
like($@, qr/^Odd number of arguments at /i);

$result = eval { Big->pseudo_rand_bits };
like($@, qr/^No bits argument at /i);

$result = eval { Big->pseudo_rand_bits(bit_length => 5, lsb_ones => -1) };
like($@, qr/^Negative number of lsb_ones at /i);

$result = eval { Big->pseudo_rand_bits(bit_length => 5, lsb_ones => 2) };
like($@, qr/^More than 1 lsb_ones is unsupported at /i);

$result = eval { Big->pseudo_rand_bits(bit_length => 5, msb_ones => -1) };
like($@, qr/^Negative number of msb_ones at /i);

$result = eval { Big->pseudo_rand_bits(bit_length => 5, msb_ones => 3) };
like($@, qr/^More than 2 msb_ones is unsupported at /i);

$result = eval { Big->pseudo_rand_bits(bit_length => 0, lsb_ones => 1) };
like($@, qr/^More lsb_ones than bits at /i);

$result = eval { Big->pseudo_rand_bits(bit_length => 1, msb_ones => 2) };
like($@, qr/^More msb_ones than bits at /i);

for (0..5) {
    $result = Big->pseudo_rand_bits(bit_length => 1, lsb_ones => 1);
    is("$result", 1);

    $result = Big->pseudo_rand_bits(bit_length => 1, msb_ones => 1);
    is("$result", 1);

    $result = Big->pseudo_rand_bits(bit_length => 2, msb_ones => 2);
    is("$result", 3);

    $result = Big->pseudo_rand_bits(bit_length => 1, msb_ones => 1, lsb_ones => 1);
    is("$result", 1);

    $result = Big->pseudo_rand_bits(bit_length => 2, msb_ones => 1, lsb_ones => 1);
    is("$result", 3);

    $result = Big->pseudo_rand_bits(bit_length => 2, msb_ones => 2, lsb_ones => 1);
    is("$result", 3);

    $result = Big->pseudo_rand_bits(bit_length => 3, msb_ones => 2, lsb_ones => 1);
    is("$result", 7);
}

my %results;
$results{Big->pseudo_rand_bits(bit_length => 2)}++ for 1..1000;
...
%results = ();
$results{Big->pseudo_rand_bits(bit_length => 2, lsb_ones => 0)}++ for 1..1000;
# Check reasonable spread (extremely small chance of random failure)
is_deeply([sort {$a <=> $b } keys %results], [0..3]);
for (0..3) {
    ok($results{$_} > 1000/4/2);
...
%results = ();
$results{Big->pseudo_rand_bits(bit_length => 2, lsb_ones => 1)}++ for 1..1000;
# Check reasonable spread (extremely small chance of random failure)
...
%results = ();
$results{Big->pseudo_rand_bits(bit_length => 2, msb_ones => 0)}++ for 1..1000;
# Check reasonable spread (extremely small chance of random failure)
is_deeply([sort {$a <=> $b } keys %results], [0..3]);
for (0..3) {
    ok($results{$_} > 1000/4/2);
...
%results = ();
$results{Big->pseudo_rand_bits(bit_length => 2, msb_ones => 1)}++ for 1..1000;
# Check reasonable spread (extremely small chance of random failure)
...
%results = ();
$results{Big->pseudo_rand_bits(bit_length => 3, msb_ones => 2)}++ for 1..1000;
# Check reasonable spread (extremely small chance of random failure)
